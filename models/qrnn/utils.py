    ...
    def forward(self, x, xs, reverse=False):
        if not self.enable_ad:            
            ...
            for i in range(num_half_layer-1):
                ...
                xs.append(x)
                np.save(
                    '/home/intern_lpang/ImageDenosing/code/Project/STQ3D/images/FeatureMaps/QRNN/encoder_qrnn_%s.npy' % str(
                        i),
                    x.detach().cpu().numpy())
...
class QRNN3DDecoder(nn.Module):
    ...
    def forward(self, x, xs, reverse=False):        
        if not self.enable_ad:
            ...
            np.save(
                '/home/intern_lpang/ImageDenosing/code/Project/STQ3D/images/FeatureMaps/QRNN/decoder_qrnn_%s.npy' % str(
                    0),
                x.detach().cpu().numpy())
            reverse = not reverse
            for i in range(1, num_half_layer):
